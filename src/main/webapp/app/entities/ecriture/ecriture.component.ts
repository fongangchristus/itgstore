...
export class EcritureComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        this.ecritureService
            ...
            .subscribe(
                (res: HttpResponse<IEcriture[]>) => this.paginateEcritures(res.body, res.headers),
    ...
    trackId(index: number, item: IEcriture) {
        return item.id;
    }

    registerChangeInEcritures() {
        this.eventSubscriber = this.eventManager.subscribe('ecritureListModification', response => this.loadAll());
    }
    ...
    private paginateEcritures(data: IEcriture[], headers: HttpHeaders) {
