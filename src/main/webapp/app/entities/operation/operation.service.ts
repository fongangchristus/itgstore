...
export class OperationService {
    ...
    protected convertDateFromClient(operation: IOperation): IOperation {
        const copy: IOperation = Object.assign({}, operation, {
            dateOperation: operation.dateOperation != null && operation.dateOperation.isValid() ? operation.dateOperation.toJSON() : null
        });
        return copy;
    }
    ...
    protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
        if (res.body) {
            res.body.forEach((operation: IOperation) => {
                operation.dateOperation = operation.dateOperation != null ? moment(operation.dateOperation) : null;
            });
        }
        return res;
    }
